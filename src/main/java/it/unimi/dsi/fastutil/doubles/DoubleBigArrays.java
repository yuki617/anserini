	...
	private static void selectionSort(final double[][] a, final double[][] b, final long from, final long to) {
	 for(long i = from; i < to - 1; i++) {
	  ...
	  for(long j = i + 1; j < to; j++)
	   if (( Double.compare((BigArrays.get(a, j)),(BigArrays.get(a, m))) < 0 ) || ( Double.compare((BigArrays.get(a, j)),(BigArrays.get(a, m))) == 0 ) && ( Double.compare((BigArrays.get(b, j)),(BigArrays.get(b, m))) < 0 )) m = j;
