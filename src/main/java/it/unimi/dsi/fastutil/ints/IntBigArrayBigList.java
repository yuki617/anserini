...
public class IntBigArrayBigList extends AbstractIntBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public static IntBigArrayBigList wrap(final int a[][], final long length) {
	 if (length > BigArrays.length(a)) throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size (" + BigArrays.length(a) + ")");
	...
	public void size(final long size) {
	 if (size > BigArrays.length(a)) a = BigArrays.forceCapacity(a, size, this.size);
	...
	public IntBigListIterator listIterator(final long index) {
	 ...
	 return new IntBigListIterator () {
	   ...
	   @Override
	   public int nextInt() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public int previousInt() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
